    ...
    $("#usuarios").select2({
      ...
      templateSelection: function (response) {
        if (response && response.text !== "Usuários") {
          return $("<span class='badge bg-success fw-normal' style='font-size:12px;'>" + response.text + "</span>");
        }
...
function templateResultProduto(usuario) {
  if (usuario.loading) {
    return $(`<span class='badge bg-success fw-normal' style='font-size:12px;'>${usuario.text}</span>`);
  }
  return $("<span class='badge bg-success fw-normal' style='font-size:12px;'>" + usuario.text + "</span>");
}
